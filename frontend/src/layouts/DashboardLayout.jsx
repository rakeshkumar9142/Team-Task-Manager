...
export const DashboardLayout = ({ children }) => {
  ...
  return (
    <div className="min-h-screen bg-bgPrimary text-slate-100">
      <div className="mx-auto grid max-w-7xl grid-cols-1 gap-6 p-6 lg:grid-cols-[260px_1fr]">
        <aside className="glass rounded-3xl p-5">
          <p className="text-xs uppercase tracking-wider text-slate-400">Team Task Manager</p>
              ...
              <NavLink
                ...
                className={({ isActive }) =>
                  `flex items-center gap-3 rounded-xl px-3 py-2 transition ${
                    isActive ? "bg-accentPurple/30 text-white" : "text-slate-300 hover:bg-white/5"
                  }`
                }
              >
                <link.icon size={16} />
          ...
          <button
            onClick={logout}
            className="mt-6 flex w-full items-center justify-center gap-2 rounded-xl border border-white/10 py-2 text-sm text-slate-200 hover:bg-white/5"
          >
            <LogOut size={16} />
            Logout
          </button>
